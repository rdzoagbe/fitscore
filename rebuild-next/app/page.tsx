    ...
    <main className="min-h-dvh bg-bg px-5 py-10 text-[var(--text-primary)]">
      <section className="mx-auto grid min-h-[calc(100dvh-5rem)] w-full max-w-6xl items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3"><Link href="/dashboard"><Button variant="primary">Open cockpit</Button></Link><Link href="/login"><Button>Sign in</Button></Link></div>
        </div>
        <Card className="border-accent/20 bg-surface/80">
          <p className="text-xs uppercase tracking-[0.18em] text-accent">Phase 1 preview</p>
          <div className="mt-6 grid gap-3">{['ATS score: 74%','Applications tracked: 47','Interview readiness: 82%','IPR dossier: Ready'].map(item => <div key={item} className="rounded-md border border-border bg-elevated px-4 py-3 text-sm text-[var(--text-secondary)]">{item}</div>)}</div>
        </Card>
      </section>
    </main>
  )
