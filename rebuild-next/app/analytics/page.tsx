        ...
        <section className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
          <Card>
            ...
            <div className="flex h-64 items-end gap-1 rounded-md border border-border bg-elevated p-4">
              {Array.from({ length: 22 }).map((_, index) => <span key={index} className="flex-1 rounded-t bg-accent/70" style={{ height: `${24 + index * 2.2}px` }} />)}
            </div>
          </Card>
          <Card>
            <CardHeader><CardTitle>By platform</CardTitle></CardHeader>
            <div className="grid gap-3"><ProgressRow label="LinkedIn" value={90} /><ProgressRow label="APEC" value={50} tone="violet" /><ProgressRow label="HelloWork" value={30} /><ProgressRow label="Cadremploi" value={25} tone="amber" /></div>
          </Card>
        </section>
      </PageScaffold>
    </AppShell>
