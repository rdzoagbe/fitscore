      ...
      <section className="grid gap-4 xl:grid-cols-[0.7fr_1.3fr]">
        <Card>
          ...
          <div className="grid gap-3">
            <Button variant="primary">Generate PDF dossier</Button>
            <Button>Export Excel history</Button>
            <Button>Export CSV raw data</Button>
          </div>
        </Card>
        <Card>
              ...
              <tbody>
                {rows.map((company, index) => (
                  <tr key={company} className="border-b border-border">
                    <td className="py-3 pr-4">{index < 3 ? '13/05/26' : '09/05/26'}</td>
                    <td className="py-3 pr-4 text-[var(--text-primary)]">{company}</td>
                    <td className="py-3 pr-4">{index % 3 === 0 ? 'Entretien' : index % 2 === 0 ? 'Refusée' : 'Envoyée'}</td>
                    <td className="py-3 pr-4">{index % 2 === 0 ? 'Oui' : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
