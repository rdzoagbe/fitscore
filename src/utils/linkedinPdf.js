...
const ACCENT = [224, 120, 86]   // E07856
const TEXT_PRIMARY = [26, 27, 34]   // 1A1B22
const TEXT_SECONDARY = [92, 96, 102] // 5C6066

export function generateOptimizedLinkedInPdf(optimization, opts = {}) {
  ...
  doc.setFontSize(20)
  doc.setTextColor(...TEXT_PRIMARY)
  doc.text('Your Optimized LinkedIn Profile', pageWidth / 2, y, { align: 'center' })
  y += 22
  doc.setFont('helvetica', 'italic')
  doc.setFontSize(10)
  doc.setTextColor(...ACCENT)
  doc.text(`Score after improvements: ${Math.min(100, (o.overall_score || 0) + 25)}/100`, pageWidth / 2, y, { align: 'center' })
  y += 18
  ...
  if (o.experience?.improvements?.length > 0) {
    ...
    o.experience.improvements.forEach(imp => {
      ...
      if (Array.isArray(imp.improved_bullets)) {
        imp.improved_bullets.forEach(b => bullet(b))
      }
    })
  }
  ...
  if (o.skills) {
    ...
    if (hasContent) {
      ...
      if (o.skills.to_reorder_top_3?.length > 0) {
        body(`Pin these as your top 3: ${o.skills.to_reorder_top_3.join(' · ')}`)
      }
      if (o.skills.to_add?.length > 0) {
        body(`Add: ${o.skills.to_add.join(', ')}`)
      }
      if (o.skills.to_remove?.length > 0) {
        body(`Remove: ${o.skills.to_remove.join(', ')}`, TEXT_SECONDARY, true)
      }
    }
  }
  ...
  if (o.quick_wins?.length > 0) {
    heading('Quick Wins')
    o.quick_wins.forEach(qw => bullet(qw))
  }
