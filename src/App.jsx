...
function ChipBtn({ onClick, children, accent }) {
  return (
    <button onClick={onClick} style={{ background: accent ? 'rgba(200,245,66,0.1)' : 'rgba(255,255,255,0.06)', border: `1px solid ${accent ? 'rgba(200,245,66,0.25)' : 'rgba(255,255,255,0.1)'}`, borderRadius: 20, padding: '7px 16px', color: accent ? '#c8f542' : '#888', fontSize: 13, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
      {children}
    </button>
  )
}
...
function AnalyzerPage({ onViewDashboard, prefillAnalysis, onClearPrefill }) {
  ...
  const [lastCvName] = useState(() => localStorage.getItem('fitscore_last_cv') || null)
  ...
  return (
    <div className="page">
      ...
      <main className="page-main">
        {displayStatus !== 'done' && (
          <div style={{ maxWidth: 540, animation: 'fadeUp 0.4s ease' }}>
            {status === 'idle' && (
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type="url" value={jobUrl} onChange={e => setJobUrl(e.target.value)}
                  ...
                  style={{ width: '100%', background: '#1f1f1f', border: `1px solid ${isValidUrl(jobUrl) ? 'rgba(200,245,66,0.3)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 10, color: '#f0f0f0', fontSize: 14, padding: '13px 14px 13px 40px', outline: 'none', transition: 'border-color 0.2s', WebkitAppearance: 'none' }}
                />
              </div>
              {jobUrl && !isValidUrl(jobUrl) && <p style={{ fontSize: 12, color: '#ff7070', marginTop: 5 }}>Please enter a valid URL including https://</p>}
              <p style={{ fontSize: 11, color: '#444', marginTop: 5 }}>Works with Indeed, WTTJ, Glassdoor. LinkedIn may block access.</p>
            </div>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              {!cvFile ? (
                <div onDrop={onDrop} onDragOver={e => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onClick={() => fileInputRef.current?.click()}
                  style={{ border: `1.5px dashed ${dragging ? 'rgba(200,245,66,0.5)' : 'rgba(255,255,255,0.12)'}`, borderRadius: 12, padding: 'clamp(20px,4vw,36px) 20px', textAlign: 'center', cursor: 'pointer', background: dragging ? 'rgba(200,245,66,0.04)' : 'rgba(255,255,255,0.02)', transition: 'all 0.2s' }}>
            ...
            {status !== 'loading' && (
              <button onClick={handleAnalyze} disabled={!canAnalyze} style={{ width: '100%', padding: '15px', borderRadius: 14, background: canAnalyze ? '#c8f542' : 'rgba(255,255,255,0.06)', color: canAnalyze ? '#0f0f0f' : '#444', border: 'none', fontFamily: 'Syne, sans-serif', fontSize: 15, fontWeight: 700, cursor: canAnalyze ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
                Run ATS check →
              </button>
            )}
          </div>
