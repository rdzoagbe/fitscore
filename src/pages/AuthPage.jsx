    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 'clamp(20px,5vw,40px)', transition: 'background 0.3s' }}>
      ...
      <div style={{ width: '100%', maxWidth: 420, animation: 'fadeUp 0.4s ease' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-muted)', letterSpacing: '0.06em' }}>KNOW BEFORE YOU APPLY</p>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 12, lineHeight: 1.6 }}>
            Beat the algorithm. Get hired.
          </p>
        </div>

        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: 'clamp(20px,5vw,32px)', transition: 'background 0.3s' }}>
          ...
          <div style={{ display: 'flex', background: 'var(--bg-input)', borderRadius: 10, padding: 3, marginBottom: 24 }}>
            {['signin','signup'].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(''); setSuccess('') }} style={{
                ...
                color: mode===m?'#0f0f0f':'var(--text-muted)',
          ...
          {/* Google */}
          <button onClick={handleGoogle} style={{ width:'100%', padding:'13px', borderRadius:12, background:'var(--bg-input)', border:'1px solid var(--border)', color:'var(--text-primary)', fontSize:14, cursor:'pointer', marginBottom:16, display:'flex', alignItems:'center', justifyContent:'center', gap:10, transition:'all 0.2s' }}>
          ...
          <input type="email" placeholder="Email address" value={email} onChange={e=>setEmail(e.target.value)} style={{ marginBottom: 10 }} />
          <input type="password" placeholder="Password (min. 6 characters)" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSubmit()} style={{ marginBottom: 18 }} />

          {error && <p style={{ fontSize:13, color:'#ff7070', marginBottom:12, lineHeight:1.4, padding:'10px 12px', background:'rgba(255,79,79,0.08)', borderRadius:8 }}>{error}</p>}
