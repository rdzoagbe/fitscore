  ...
  return (
    <div style={{ ...cardStyle({ borderRadius: 18, padding: '13px 15px', boxShadow: 'none' }), display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        {!example && (
          <button onClick={copyTip} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: copiedTip ? theme.green : theme.muted, flexShrink: 0, padding: '2px 4px' }} title={t('copy')}>
            {copiedTip ? '✓' : '📋'}
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ ...cardStyle({ overflow: 'hidden', marginBottom: 16 }) }}>
      ...
      <div style={{ padding: 18, display: 'grid', gap: 18 }}>
        {qw.length > 0 && (
          <div>
            <p style={{ fontSize: 10, fontWeight: 900, color: theme.copper, letterSpacing: '0.10em', textTransform: 'uppercase', marginBottom: 10 }}>
              ✏️ {t('quick_wins')} — {t('coach_add_hint')}
            </p>
            <div style={{ display: 'grid', gap: 9 }}>{qw.map((w, i) => <QuickWinCard key={i} win={w} index={i} t={t} />)}</div>
          </div>
        )}
        ...
        {missing.length > 0 && (
          <div>
            <p style={{ fontSize: 10, fontWeight: 900, color: theme.copper, letterSpacing: '0.10em', textTransform: 'uppercase', marginBottom: 8 }}>
              🏷️ {t('missing')} {t('keywords')} — {t('coach_keyword_hint')}
            </p>
        ...
        {gaps.length > 0 && (
          <div>
            <p style={{ fontSize: 10, fontWeight: 900, color: theme.red, letterSpacing: '0.10em', textTransform: 'uppercase', marginBottom: 8 }}>⚠️ {t('gaps')} — {t('coach_gaps_hint')}</p>
            <div style={{ display: 'grid', gap: 7 }}>
...
export default function CvCoachPage() {
  ...
  const generateCoverLetter = async () => {
    ...
    try {
      ...
      if (!nameToUse) {
        setEditingName(true)
        setGenError(t('name_required_first'))
        setGenLoading(false)
        return
      }
    ...
    <div style={{ ...pageVars, minHeight: '100dvh', background: `radial-gradient(circle at 16% 8%, rgba(181,102,60,0.12), transparent 34%), linear-gradient(180deg, ${theme.ivory} 0%, #F8F1E8 48%, ${theme.ivory} 100%)`, padding: 'clamp(20px,4vw,38px) clamp(16px,5vw,48px) 90px' }}>
      <main style={{ maxWidth: 1120, margin: '0 auto' }}>
        <div style={{ ...cardStyle({ padding: 'clamp(24px,4vw,42px)', marginBottom: 22, borderRadius: 30 }) }}>
          <p style={{ margin: 0, color: theme.copper, fontSize: 11, fontWeight: 950, letterSpacing: '0.14em', textTransform: 'uppercase' }}>{t('coach_kicker')}</p>
          ...
          <div>
            <div style={{ ...cardStyle({ overflow: 'hidden', position: 'sticky', top: 86, marginBottom: 16 }) }}>
              <div style={{ padding: '16px 18px', borderBottom: `1px solid ${theme.line}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div>
                  <p style={{ fontSize: 19, fontWeight: 500, color: theme.navy, fontFamily: 'Georgia, Newsreader, serif', marginBottom: 2, letterSpacing: '-0.04em' }}>✉️ {t('cover_letter')}</p>
                  <p style={{ fontSize: 12, color: theme.muted }}>{t('cover_letter_desc')}</p>
                </div>
                {coverLetter && <button onClick={copyLetter} style={{ padding: '8px 14px', borderRadius: 999, background: copied ? 'rgba(85,124,100,0.15)' : 'rgba(255,255,255,0.62)', border: `1px solid ${copied ? 'rgba(85,124,100,0.3)' : theme.line}`, color: copied ? theme.green : theme.muted, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 800 }}>{copied ? `✓ ${t('copied')}` : `📋 ${t('copy')}`}</button>}
              </div>

              <div style={{ padding: 18 }}>
                {!coverLetter && !genLoading && (
                  <div style={{ padding: '6px 2px' }}>
                    <p style={{ fontSize: 13, color: theme.muted, marginBottom: 16, lineHeight: 1.6, textAlign: 'center' }}>✉️ {t('cover_letter_prompt')}</p>

                    <div style={{ marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                        <span style={{ fontSize: 10, fontWeight: 900, color: theme.copper, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{t('your_name')}</span>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <span style={{ fontSize: 10, fontWeight: 900, color: theme.copper, letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>{t('recipient_label')} <span style={{ textTransform: 'none', fontWeight: 500, color: theme.muted }}>· {t('optional')}</span></span>
                      <input type="text" value={recipient} onChange={e => setRecipient(e.target.value)} placeholder={t('recipient_placeholder')} style={{ width: '100%', padding: '9px 12px', fontSize: 13, background: 'rgba(255,255,255,0.62)', border: `1px solid ${theme.line}`, borderRadius: 12, color: theme.navy }} />
                    </div>

                    <p style={{ fontSize: 10, fontWeight: 900, color: theme.copper, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>{t('cover_letter_tone')}</p>
                    ...
                    <p style={{ fontSize: 10, fontWeight: 900, color: theme.copper, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>{t('cover_letter_length')}</p>
