...
export function ThemeProvider({ children }) {
  const [theme, setTheme] = useState(() => localStorage.getItem('fitscore_theme') || 'system')
  ...
  const [effective, setEffective] = useState(() => getEffective(localStorage.getItem('fitscore_theme') || 'system'))
  ...
  useEffect(() => {
    ...
    if (effective === 'dark') {
      ...
      root.style.setProperty('--text-hint', '#444')
      root.style.setProperty('--shadow', 'rgba(0,0,0,0.4)')
    } else {
      root.style.setProperty('--bg', '#f5f5f5')
      root.style.setProperty('--bg-card', '#ffffff')
      root.style.setProperty('--bg-input', '#eeeeee')
