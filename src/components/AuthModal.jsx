...
export default function AuthModal({ initialMode = 'signin', onClose, initialError = '' }) {
  ...
  const handleSubmit = async () => { setError(''); setSuccess(''); if (!requireLegalForSignup()) return; setLoading(true); try { if (mode === 'signin') { const { error } = await signIn(email, password); if (error) throw error; window.location.href = window.location.origin + '/dashboard' } else { const { error } = await signUp(email, password, 'signup_email_checkbox'); if (error) throw error; setSuccess(t('account_created')) } } catch (e) { setError(e.message) } setLoading(false) }
  const requireLegalForOAuth = () => { if (mode === 'signup' && !acceptedLegal) { setError(t('legal_required_oauth')); return false } return true }
  const handleGoogle = async () => { setError(''); if (!requireLegalForOAuth()) return; setOauthLoading('google'); try { const { error } = await signInWithGoogle(mode === 'signup' ? 'signup_google_checkbox' : 'signin_google'); if (error) setError(error.message || 'Google sign-in failed. Check your Supabase Google OAuth configuration.') } catch (e) { setError(e?.message || 'Google sign-in failed. Please try again.') } finally { setOauthLoading('') } }
  const handleMicrosoft = async () => { setError(''); if (!requireLegalForOAuth()) return; setOauthLoading('microsoft'); try { const { error } = await signInWithMicrosoft(mode === 'signup' ? 'signup_microsoft_checkbox' : 'signin_microsoft'); if (error) setError(error.message || 'Microsoft sign-in failed. Check your Supabase Azure OAuth configuration.') } catch (e) { setError(e?.message || 'Microsoft sign-in failed. Please try again.') } finally { setOauthLoading('') } }
  const handleLinkedIn = async e => { e.preventDefault(); setError(''); if (!requireLegalForOAuth()) return; setOauthLoading('linkedin'); try { const { error } = await signInWithLinkedIn(mode === 'signup' ? 'signup_linkedin_checkbox' : 'signin_linkedin'); if (error) setError(error.message || 'LinkedIn sign-in failed. Check your Supabase LinkedIn OAuth configuration.') } catch (e) { setError(e?.message || 'LinkedIn sign-in failed. Please try again.') } finally { setOauthLoading('') } }
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'fadeIn 0.2s ease' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: 'clamp(24px,5vw,32px)', maxWidth: 460, width: '100%', position: 'relative', animation: 'fadeUp 0.3s ease', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <button onClick={handleGoogle} disabled={!!oauthLoading} style={{ width: '100%', padding: '12px', borderRadius: 12, background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontSize: 14, cursor: oauthLoading ? 'not-allowed' : 'pointer', opacity: oauthLoading && oauthLoading !== 'google' ? 0.5 : 1, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}><svg width="18" height="18" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>{oauthLoading === 'google' ? 'Connecting…' : t('continue_google')}</button>
        <button type="button" onClick={handleMicrosoft} disabled={!!oauthLoading} style={{ width: '100%', padding: '12px', borderRadius: 12, background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontSize: 14, cursor: oauthLoading ? 'not-allowed' : 'pointer', opacity: oauthLoading && oauthLoading !== 'microsoft' ? 0.5 : 1, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}><span style={{ width: 18, height: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: 2 }}><i style={{ background: '#f25022' }} /><i style={{ background: '#7fba00' }} /><i style={{ background: '#00a4ef' }} /><i style={{ background: '#ffb900' }} /></span>{oauthLoading === 'microsoft' ? 'Connecting…' : t('continue_microsoft', 'Continue with Microsoft / Outlook')}</button>
        <button type="button" onClick={handleLinkedIn} disabled={!!oauthLoading} style={{ width: '100%', padding: '12px', borderRadius: 12, background: '#0A66C2', border: '1px solid #0A66C2', color: '#fff', fontSize: 14, fontWeight: 600, cursor: oauthLoading ? 'not-allowed' : 'pointer', opacity: oauthLoading && oauthLoading !== 'linkedin' ? 0.5 : 1, marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}><span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 20, height: 20, borderRadius: 4, background: '#fff', color: '#0A66C2', fontWeight: 800, fontSize: 13, lineHeight: 1 }}>in</span>{oauthLoading === 'linkedin' ? 'Connecting…' : t('continue_linkedin', 'Continue with LinkedIn')}</button>
