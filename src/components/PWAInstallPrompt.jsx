...
export default function PWAInstallPrompt() {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 32px)', maxWidth: 400, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '14px 16px', zIndex: 500, display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 8px 32px var(--shadow)', animation: 'fadeUp 0.4s ease' }}>
      ...
      <div style={{ flex: 1 }}>
        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 2 }}>Install FitScore</p>
        <p style={{ fontSize: 11, color: 'var(--text-secondary)' }}>Add to home screen for quick access</p>
      </div>
      ...
      <button onClick={handleInstall} style={{ background: 'var(--accent)', border: 'none', borderRadius: 10, padding: '8px 14px', color: '#0f0f0f', fontSize: 12, fontWeight: 700, cursor: 'pointer', flexShrink: 0, fontFamily: 'Syne, sans-serif' }}>Install</button>
    </div>
  )
}
