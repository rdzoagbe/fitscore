...
function encrypt(value) {
  const secret = process.env.MICROSOFT_TOKEN_ENCRYPTION_KEY || process.env.GOOGLE_TOKEN_ENCRYPTION_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.STRIPE_SECRET_KEY
  if (!secret) throw new Error('MICROSOFT_TOKEN_ENCRYPTION_KEY is not configured.')
...
async function exchangeCodeForTokens({ code, redirectUri }) {
  ...
  if (!response.ok) throw new Error(data?.error_description || data?.error || 'Could not exchange Microsoft authorization code.')
  return data
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error) return res.redirect(`${appUrl}/messages?sync=cancelled&provider=microsoft&reason=${encodeURIComponent(error)}`)
    if (!code || !state) return res.redirect(`${appUrl}/messages?sync=failed&provider=microsoft&reason=missing_oauth_code`)
    ...
    const { error: upsertError } = await supabase
      ...
      .upsert({
        ...
        provider: 'microsoft',
        provider_email: profile?.mail || profile?.userPrincipalName || statePayload.email || null,
    ...
    return res.redirect(`${appUrl}/messages?sync=connected&provider=microsoft`)
  } catch (e) {
    console.error('Microsoft sync callback error:', e)
    return res.redirect(`${appUrl}/messages?sync=failed&provider=microsoft&reason=${encodeURIComponent(e.message || 'sync_failed')}`)
  }
}
