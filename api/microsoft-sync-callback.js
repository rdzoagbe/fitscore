...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (oauthError) {
      return redirectToMessages(res, appUrl, { sync: 'failed', provider: 'microsoft', reason: query.error_description || oauthError })
    }

    const code = query.code
    const state = query.state
    if (!code || !state) return redirectToMessages(res, appUrl, { sync: 'failed', provider: 'microsoft', reason: 'missing_oauth_code' })
