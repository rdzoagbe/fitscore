...
function redirectToMessages(req, res, params) {
  const url = new URL('/messages', getAppOrigin(req))
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return redirectToMessages(req, res, { sync: 'connected', provider })
  } catch (error) {
    console.error('Mail sync callback error:', error)
    return redirectToMessages(req, res, { sync: 'failed', reason: error.message || 'Mail/calendar sync could not be completed.' })
  }
}
