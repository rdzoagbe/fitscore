...
export function classifyAiError(error, feature = 'analysis') {
  ...
  if (status === 401 || status === 403 || lower.includes('api key')) {
    return {
      ...
      retryable: false,
      providerMessage,
      userMessage: 'The AI provider is not available because the server credentials need attention. Please try again later.'
    }
  }
  ...
  if (status === 429 || type.includes('rate_limit') || lower.includes('rate limit')) {
    return {
      ...
      retryable: true,
      providerMessage,
      userMessage: 'The AI provider is temporarily busy. Please wait a moment and try again.'
    }
  }
  ...
  if (status === 402 || lower.includes('credit') || lower.includes('billing') || lower.includes('balance')) {
    return {
      ...
      retryable: false,
      providerMessage,
      userMessage: 'AI processing is temporarily unavailable. The service owner needs to refresh the AI provider billing or credits.'
    }
  }
  ...
  if (status === 400 || lower.includes('too long') || lower.includes('maximum context') || lower.includes('prompt is too long')) {
    return {
      ...
      retryable: false,
      providerMessage,
      userMessage: feature === 'cover_letter'
        ? 'The cover letter context is too long. Try generating it from a shorter analysis or remove extra notes.'
        : 'The job description or CV content is too long for the AI model. Paste a shorter version focused on responsibilities and requirements.'
    }
  }
  ...
  if (status === 529 || lower.includes('overloaded') || lower.includes('temporarily')) {
    return {
      ...
      retryable: true,
      providerMessage,
      userMessage: 'The AI provider is overloaded right now. Please try again in a few minutes.'
    }
  }
  ...
  return {
    ...
    providerMessage,
    userMessage: feature === 'cover_letter'
      ? 'Cover letter generation failed because the AI provider returned an error. Please try again.'
...
export function buildFallbackAnalysis({ jobText, cvText, jobUrl = null }) {
  ...
  return {
    ...
    seniority: {
      candidate_level: 'mid',
      job_level: 'mid',
      alignment: 'right_level',
      alignment_label: 'Needs review',
      alignment_reason: 'Fallback mode cannot reliably assess seniority.',
      candidate_years: 0,
